...
use crate::utils::{BusRequests, LTARequest};
...
impl<'a> BusRequests<reqwest::RequestBuilder> for Bus {
    ...
    async fn get_bus_services(
        ...
        build_req_async_with_skip::<bus_services::BusServiceResp, _, _>(c, bus_services::URL, skip)
            .await
    }

    async fn get_bus_routes(c: &Self::ClientType, skip: Option<u32>) -> LTAResult<Vec<BusRoute>> {
        build_req_async_with_skip::<bus_routes::BusRouteResp, _, _>(c, bus_routes::URL, skip).await
    }

    async fn get_bus_stops(c: &Self::ClientType, skip: Option<u32>) -> LTAResult<Vec<BusStop>> {
        build_req_async_with_skip::<bus_stops::BusStopsResp, _, _>(c, bus_stops::URL, skip).await
    }
}
